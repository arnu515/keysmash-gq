...
export const post: RequestHandler = async ({ body, locals, headers, params }) => {
  ...
  const {error: fError, data: fData} = await supabase.rpc<{id: string, email: string}>("get_user_from_jwt", {jwt: headers["x-token"]}).select()
  if (fError || !fData[0]) {
    return { status: 401, body: { error: fError?.message || "Unauthorized"} };
  }
  const user = fData[0];
  
  const {error: cError, data: cData} = await supabase.from("courses").select().eq("id", params.id)
  if (cError || !cData[0]) {
